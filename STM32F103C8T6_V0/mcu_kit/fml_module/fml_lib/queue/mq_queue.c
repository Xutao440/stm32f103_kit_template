 ...
 * @FilePath: mq_queue.c
 * @Author: jack
 * @Date: 2023-05-15 08:59:40
 * @LastEditors: 
 * @LastEditTime: 2023-05-15 09:45:09
...
#define QUEUE_COUNT_MAX 15
#define QUEUE_MSG_COUNT 55
#define QUEUE_MSG_SIZE  45
//static mqQueue queue[QUEUE_COUNT_MAX];
//static uint8_t queue_idx = 0;
//static uint8_t queue_arae[QUEUE_COUNT_MAX * (QUEUE_MSG_COUNT * QUEUE_MSG_SIZE)];
static mqQueue queue[QUEUE_COUNT_MAX]__attribute__((at(0x22000000+PLATFORM_LCD_HEIGHT * PLATFORM_LCD_WIDTH*2*3)));
static uint8_t queue_idx = 0;
static uint8_t queue_arae[QUEUE_COUNT_MAX * (QUEUE_MSG_COUNT * QUEUE_MSG_SIZE)]__attribute__((at(0x22000000+PLATFORM_LCD_HEIGHT * PLATFORM_LCD_WIDTH*2*2)));
...
mqd_t mq_open(const char* name, int oflag, mode_t mode, struct mq_attr* attr) {
    if (oflag & O_CREAT) {
        ...
        if (attr->mq_msgsize > QUEUE_MSG_SIZE || attr->mq_maxmsg > QUEUE_MSG_COUNT) {
            mq_log("mq_open error:msg_size ro msg_count to big\r\n");
            return -1;
        }
...
int mq_send(mqd_t msgid, const char* msg, size_t msg_len, unsigned int msg_prio) {
    ...
    if (msgid <= 0) {
      log_debug("Amq_send error: msgid=%d\r\n", msgid);
        return -1;
    }
    ...
    if (q->capacity >= QUEUE_MSG_COUNT) {
        log_debug("mq_send error, mq full!");
        q->capacity = QUEUE_MSG_COUNT;
        return -2;
    }

    if (msg_len > QUEUE_MSG_SIZE) {
        log_debug("msg_len to long,max=%d", QUEUE_MSG_SIZE);
        return -1;
    }
		q->lock = 1;
...
ssize_t mq_receive(mqd_t msgid, char* msg, size_t msg_len, unsigned int* msg_prio) {
    mqQueue* q;
    if (msgid <= 0 || msg == NULL) {
        //log_debug("mq_receive error: msgid=%d, or msg null\r\n", msgid);
        return -1;
    }
...
mqd_t mq_queue_test_init(void) {
    ...
    qattr.mq_maxmsg  = 5;
    qattr.mq_msgsize = 20;
    queue            = mq_open("msg_test", O_CREAT, 0644, &qattr);
...
void mq_queue_test(mqd_t queue) {
    ...
    mq_send(queue, str1, sizeof(str1), 0);
    mq_log("mq_send str1=%s, len=%d\r\n", str1, sizeof(str1));

    // 3, receive msg
    char str2[20];
    int  str2_len;
    str2_len = mq_receive(queue, str2, sizeof(str2), NULL);
