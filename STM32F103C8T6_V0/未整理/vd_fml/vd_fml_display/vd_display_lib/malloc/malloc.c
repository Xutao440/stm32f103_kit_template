...
static __align(64) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0x22000000)));     /* 外扩SRAM内存池 */

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                                      /* 内部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0x22000000 + MEM2_MAX_SIZE)));      /* 外扩SRAM内存池MAP */
